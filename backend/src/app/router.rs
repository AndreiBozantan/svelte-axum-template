...
pub fn create_router(context: core::ArcContext) -> Router {
    ...
    let auth_routes = Router::new()
        ...
        .route("/session", get(routes::auth::session)) // check session status
...
) -> Result<Response, routes::auth::AuthError> {
    let claims = auth::decode_token_from_req(&context, &req, auth::TokenType::Access).map_err(|err| {
        tracing::warn!(%err, "Unauthorized access attempt");
        err
    })?;
