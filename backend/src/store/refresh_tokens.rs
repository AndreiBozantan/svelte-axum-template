...
impl RefreshTokens {
    ...
    pub async fn insert(&self, new_refresh_token: NewRefreshToken) -> Result<(), DbError> {
        sqlx::query!(
            r#"
    ...
    pub async fn get_by_jti(&self, jti: &str) -> Result<RefreshToken, DbError> {
    ...
    pub async fn revoke_all_for_user(&self, user_id: i64) -> Result<SqliteQueryResult, DbError> {
    ...
    pub async fn _cleanup_expired(&self) -> Result<SqliteQueryResult, DbError> {
        let now = Utc::now().naive_utc();
        let result = sqlx::query!(
            r#"
